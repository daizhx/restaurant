...
public class JIpAddressField extends JComponent implements SwingConstants {
	...
	public List<JTextField> getTxts() {
		List<JTextField> ret = new Vector<JTextField>();
		ret.add(txtDigital1);
		ret.add(txtDigital2);
		ret.add(txtDigital3);
		ret.add(txtDigital4);
	...
	private int isChangeTextFocus(DigitalText text, KeyEvent e) {
		...
		if (selectText == null && length > 0
				&& (e.getKeyCode() == 46 || e.getKeyCode() == 110)) {// 110,46代表'.'
			return 1;
		}
		return 0;
	}
